...
export default function InvoiceForm({
  ...
  const CustomerView = () => (
    <View style={styles.container}>
      <StepHeader subtitle="1/4" title={Translations.CUSTOMER} progress={0.25} />
      <KeyboardAwareScroll>
  ...
  const ItemsView = () => (
    <View style={styles.container}>
      <StepHeader subtitle="2/4" title={Translations.INVOICE_LINES} progress={0.5} />
  ...
  const DetailsView = () => (
    <View style={styles.container}>
      <StepHeader subtitle="3/4" title={Translations.INVOICE_DETAILS} progress={0.75} />
      <View ref={setDetailsTopClearFix} />
      <KeyboardAwareScroll innerRef={setDetailsScroll}>
        {accountDetails.bank_accounts.length > 1 && (
          <SelectorInput
            label={Translations.BANK_ACCOUNT}
            onSelect={(value) => emitChanges({bank_account_id: value})}
            value={formData.bank_account_id}
            onPress={openSelector}
            error={getFieldError(error, 'bank_account_id')}
            topBorder
            data={accountDetails.bank_accounts.map((item) => {
              return {
                label: `${item.account_name} (${item.iban})`,
                value: item.id,
              };
            })}
          />
        ...
        <DateSelectorInput
          label={Translations.INVOICE_DATE}
          value={formData.invoice_date}
          onSelect={(value) => emitChanges({invoice_date: value})}
          error={getFieldError(error, 'invoice_date')}
        />

        <SelectorInput
          label={Translations.TERMS_OF_PAYMENT}
          value={formData.terms_of_payment}
          onSelect={(value) => emitChanges({terms_of_payment: value})}
          error={getFieldError(error, 'terms_of_payment')}
        ...
        <PlainTextInput
          label={Translations.YOUR_REFERENCE}
          value={formData.your_reference}
          onEdit={(value) => emitChanges({your_reference: value})}
          error={getFieldError(error, 'your_reference')}
          maxLength={50}
        />
  ...
  const PreviewView = () => (
    <View style={styles.container}>
      <StepHeader subtitle="4/4" title={Translations.PREVIEW} progress={1} />
      <View style={styles.container}>
        {state === RequestStates.PREVIEW_LOADING ? (
          <Loading text={Translations.LOADING_PREVIEW} />
        ) : (
          <Pdf
            source={{
              uri: 'data:application/pdf;base64,' + previewData,
            }}
            style={styles.pdf}
          />
        )}
